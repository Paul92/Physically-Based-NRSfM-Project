...
% Last modified by D. Henrion on 31 October 2013
...
if ~isempty(mmoms) | ~isempty(msups)
 ...
 if ~any(any(triu(Ar(:,2:end),1))) | ~any(any(tril(Ar(:,2:end),-1)))
  ...
  error(['Cannot implement the moment substitutions.' 13 ...
  'Please replace substitutions constraints with equality constraints' 13 ...
  'by putting both sides of the equation on the same side.']);
 end
