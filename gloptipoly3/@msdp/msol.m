...
% Last modified on 15 January 2014 by D. Henrion
...
if feas % If SDP problem is feasible
 ...
 % only one measure with no moment or support substitution
 polyopt = (nmeas == 1) & (size(P.Ar,1) == length(P.indep)+1);
 ...
 if ~globopt
  ...
  for m = 1:nmeas
   ...
   if MMM.M{mm}.mass < eps
    ...
    while (k < kmax) & ~rankcheck(m)
     ...
     MMM.M{mm}.U = U{k}(:,1:rankM(k))*diag(sqrt(S{k}(1:rankM(k)))); %MMM.M{mm}.U = U{k}(:,1:rankM(k));
  ...
  if nbsol > 0
   ...
   if polyopt
    % for standard polynomial optimization, check objective and feasibility
    for mm = P.indmeas
     MMM.M{mm}.sol = MMM.M{mm}.sol(:,:,(reach & feas));
    end
   else
    % for generalized problems of moments, check only feasibility
    for mm = P.indmeas
     MMM.M{mm}.sol = MMM.M{mm}.sol(:,:,feas);
    end
   end
...
% Output arguments
if nargout > 0
 varargout = {status,P.sdpobj,M,dual_vars}; % modified by M. Korda
end
