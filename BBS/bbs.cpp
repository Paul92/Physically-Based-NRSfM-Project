...
/*    if (nb_x == 400) {
        mexPrintf("ninter: %d        width_inter: %lf\n", ninter, width_inter);
        mexPrintf("xmin:   %lf       xmax:        %lf\n", xmin, xmax);
        mexPrintf("x: ");
        for (int i = 0; i < 400; i++)
            mexPrintf("%lf ", x[i]);
        mexPrintf("\n");
...
/*    mexPrintf(">> Getting deriv fact for order u%d v%d\n", uorder, vorder);
    
    mexPrintf(">> Values:\n");
    mexPrintf(">> umin %g umax %g vmin %g vmax %g\n", bbs->umin, bbs->umax, bbs->vmin, bbs->vmax);
    mexPrintf(">> nptsu %d nptsv %d\n", bbs->nptsu, bbs->nptsv);
*/    
...
//        mexPrintf(">> su %g sg %g\n\n\n", su, sv);
    ...
    double a = pow(su,uorder);
    double b = pow(sv,vorder);
    double p = a * b;
    
//    mexPrintf(">> a %g b %g p %g\n", a, b, p);
...
void eval(bbs_t *bbs, double *ctrlpts, double *u, double *v, int nval, double *val, int du, int dv) {
 ...
 /*   if (nval == 400 && du == 1 && dv == 0) {
        FILE *a = fopen("bbs_eval_du_cpp_nu.txt", "w");
        FILE *b = fopen("bbs_eval_du_cpp_nv.txt", "w");
        FILE *c = fopen("bbs_eval_du_cpp_interu.txt", "w");
        FILE *d = fopen("bbs_eval_du_cpp_interv.txt", "w");
        ...
        for (int i = 0; i < nval; i++) {
            fprintf(a, "%f ", nu[i]);
            fprintf(b, "%f ", nv[i]);
            fprintf(c, "%f ", interu[i]);
            fprintf(d, "%f ", interv[i]);
        }
        fclose(a);
        fclose(b);
        fclose(c);
...
int coloc(bbs_t *bbs, double *u, double *v, int nsites, double *pr, mwIndex *ir, mwIndex *jc) {
 ...
 /*   mexPrintf("Nsites: %d\n", nsites);
  ...
  /*  
        if (nsites == 400) {
        for (int i = 0; i < 400; i++)
            mexPrintf("%d ", interu[i]);
        mexPrintf("\n");
        for (int i = 0; i < 400; i++)
            mexPrintf("%d ", interv[i]);
        mexPrintf("\n\n");
    }*/
...
/*
        mexPrintf("FACT %10.10g\n", fact);
        mexPrintf("du dv\n", du, dv);
        mexPrintf("u: %g %g %g %g %g %g\n", u[0], u[1], u[2], u[3], u[4], u[5]);
        mexPrintf("v: %g %g %g %g %g %g\n", v[0], v[1], v[2], v[3], v[4], v[5]);
        mexPrintf("\n");
 */  
...
/*    
    if (du == 2) {
        mexPrintf("interu: %g %g %g %g %g %g\n", interu[0], interu[1], interu[2], interu[3], interu[4], interu[5]);
        mexPrintf("interv: %g %g %g %g %g %g\n", interv[0], interv[1], interv[2], interv[3], interv[4], interv[5]);
    }
 */   
    // FILLING jc
    // First, compute the number of elements in each column
